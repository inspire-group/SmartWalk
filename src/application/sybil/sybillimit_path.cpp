	...
	public:
		...
		void generateSrcRandomRoutes(vertex source, bool verifier, int seed){
			...
			for(int i=0;i<num_instances;i++){
				...
				next_hop=*iter;
				int len=walk_len[cur_hop];
				while(len>0){
					tmp_len++;
					len--;
					prev_hop=cur_hop;
					cur_hop=next_hop;
					if (walk_len[cur_hop]<len) len=walk_len[cur_hop];
...
int main(int argc, char *argv[]){
    ...
    int num_nodes=29060;
    walk_len=new int[num_nodes];
    ifstream in;
    stringstream ss;
    ss << g.wlen;
    string fname="./length/length_facebook_wall2_0."+ss.str()+".txt";
	...
	int sum_len=0;
	for (vector<int>::iterator it=total_len.begin();it!=total_len.end();it++) {
		sum_len+=*it;
	}
	cout << "Average walk length: "<<(double)sum_len/total_len.size() << endl;
